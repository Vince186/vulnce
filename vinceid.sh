...
+1765761-5125/+62895396843672
 ...
 printf "
           uppsss.....Upgrade Your Face :(
 "
 fi
 
 read -p "{PASSWORD} :" pass
 if [ $pass = "vincegans" ]; then
 clear
 else
 printf "
          uppssss.....Upgrade your face :(
 "
 fi
...
curl 
https://api.hackertarget.com/hostsearch/?q=$web
fi
if [ $cmd = "header" ]
then
echo $w
...
curl 
https://api.hackertarget.com/httpheaders/?q=$web1
echo
fi
if [ $cmd = "xploit" ]
then
...
clear
echo $w"Your Target ( Ex : https://www.target.com / 
http://target.com  )"
echo $g
...
curl 
https://api.hackertarget.com/geoip/?q=$target_ip
echo
fi
if [ $cmd = "dns" ]
then
...
curl 
https://api.hackertarget.com/dnslookup/?q=$target_dns
echo
fi
